...
summary(lm2)   #  Adjusted R-squared:  0.6821 -> 비슷하니까 두개만 쓰겠다
################################## #####################################
par(mfrow=c(2,2))
plot(lm) 
# 1. 분산이 점점 커지고 있음 (독립 변수 값의 크기가 커지면 종속 변수 값의 분산도 커지는 이분산성)
# 2. 정규성 오른쪽 끝 뺴고는 대체로 만족하는 것 같음(-3~2는 만족)
...
head( scores[-c(151,150,155,149,156,157,158),])
?lm
lm.cutoff1 = lm(target~.,  scores2[-which(abs(rstudent(lm2)) > cutoff) ,]) 
summary(lm.cutoff1 )  # Adjusted R-squared:  0.7305 
plot(lm.cutoff1)
...
library(boot)
bootresults<- boot(statistic = bootReg, formula =target~RC3+RC2, data=scores,   R = 2000)
boot.ci(bootresults, index=1)
L <- empinf(bootresults, index = index)
...
scores$fitted<-lm.cutoff3$fitted.values
nrow(scores)
